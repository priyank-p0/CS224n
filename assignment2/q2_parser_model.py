...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("hiddenLayer"):
            self.W= tf.get_variable("W",shape=(self.config.n_features*self.config.embed_size,self.config.hidden_size),dtype=tf.float32)
            self.b1=tf.get_variable("b1",dtype=tf.float32,initializer=tf.zeros(shape=(self.config.hidden_size,),dtype=tf.float32))
            self.U= tf.get_variable("U",shape=(self.config.hidden_size,self.config.n_classes),dtype=tf.float32)
            self.b2= tf.get_variable("b2",dtype=tf.float32,initializer=tf.zeros(shape=(self.config.n_classes,),dtype=tf.float32))
