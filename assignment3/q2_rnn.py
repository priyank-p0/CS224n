...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        additional_length = max_length-len_sentence
        if additional_length > 0:
            padded_sentence = sentence +([zero_vector]*additional_length)
            padded_labels = labels+ ([zero_label]*additional_length)
            mark = [True]*len_sentence
            mark.extend([False]*additional_length)
        else:
            mark = [True]*max_length
            padded_sentence = sentence[:max_length]
            padded_labels = labels[:max_length]

        ret.append((padded_sentence,padded_labels,mark))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE (~4-6 lines)
        embeddings = tf.nn.embedding_lookup(tf.Variable(self.pretrained_embeddings),
                                            self.input_placeholder)
